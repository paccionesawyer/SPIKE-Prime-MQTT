...
Description: Once saved to the ESP this file runs as soon as the board is
             provided power and boot.py has finished running.
Purpose:     Describe how the MQTT client on the ESP8266 should handle messages
...
def sub_cb(topic, msg):
    '''
    The callback function that is called whenever client.check_msg() or client.
    wait_msg() finds a message to a topic the ESP is subscribed to
    '''
...
def connect_and_subscribe():
    '''
    Helper function to connect this client to a MQTT Broker and Subscribe to 
    the topics specified in the MQTT CONFIG file
    '''
    global client
...
def spike_checkMSG():
    '''
    Check if there is a pending message from server. If yes, process the same 
    way as spike_waitMSG(), if not, return immediately. (REPL MUST BE OFF)
    '''
    response = uart.readline()
    if response != b'':
        spike_write(">>>") # Immediately send to EOL statement to return from dongle.ask()
...
def spike_write(msg):
    '''
    Send a message to the Spike Prime over uart communication (REPL can be on 
    or off)
    '''
    uart.write(msg)
...
def setup_publish(client):
    '''
    Create an array of all of the publish topics and publishes a simple message 
    to all of them
    '''
    for key in MQTT_CONFIG:
        if key.startswith("PUB") and str(type(MQTT_CONFIG[key])) == "<class \
        'bytes'>" and MQTT_CONFIG[key] != b'':
...
def main():
    ...
    uos.dupterm(None, 1) # Detach the REPL from UART0
    while True:
